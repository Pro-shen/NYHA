...
@RestController
@RequestMapping("/nyha/tVisitDetails")
public class TVisitDetailsController extends BaseController {

    @Autowired
    private ITVisitDetailsService itVisitDetailsService;
    ...
    public TableDataInfo list(TVisit tVisit){
        startPage();
        List<TVisit> tVisits = itVisitDetailsService.list(tVisit);
    ...
    public AjaxResult getUserId(@PathVariable("userName") String userName){
        ...
        ajaxResult.put("data",itVisitDetailsService.getUserId(userName));
        return ajaxResult;
    }
    ...
    public AjaxResult add(@Validated @RequestBody TVisit tVisit){
        ...
        ajax.put("data",itVisitDetailsService.add(tVisit));
        return ajax;
    }
    ...
    public AjaxResult selectTVisitListById(@PathVariable("id") Long id){
        ...
        ajax.put("data",itVisitDetailsService.selectTVisitListById(id));
        return ajax;
    }
    ...
    public AjaxResult edit(@RequestBody TVisit tVisit){
        AjaxResult ajax = AjaxResult.success();
        ajax.put("data",itVisitDetailsService.edit(tVisit));
        return ajax;
    }
    ...
    public AjaxResult remove(@RequestBody TVisit tVisit){
        AjaxResult ajax = AjaxResult.success();
        ajax.put("data",itVisitDetailsService.remove(tVisit));
        return ajax;
    }

    @Log(title = "就诊表单", businessType = BusinessType.EXPORT)
    @PreAuthorize("@ss.hasPermi('nyha:tVisitApproval:export')")
    @PostMapping("/export")
    public void export(HttpServletResponse response, TVisit tVisit)
    {
        List<TVisit> list = itVisitDetailsService.list(tVisit);
